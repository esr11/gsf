...
function populateSubcityFilter() {
    ...
    subcityFilter.innerHTML = '<option value="">ሁሉም ክፍለ ከተሞች</option>';
...
function populateOfficeFilter(subcityId) {
    ...
    officeFilter.innerHTML = '<option value="">ሁሉም ጽ/ቤቶች</option>';
        ...
        offices.forEach(office => {
            office.employees.forEach(emp => {
                ...
                card.innerHTML = `
                  ...
                  <h5>${emp.full_name}</h5>
                  <p><strong>ሚና:</strong> ${emp.position}</p>
                  <p><strong>ጽ/ቤት:</strong> ${office.office_name}</p>
                  <p><strong>ክፍለ ከተማ:</strong> ${subcity.subcity_name}</p>
                  <a href="feedbacka.html?employee_id=${emp.employee_id}&employee_name=${encodeURIComponent(emp.full_name)}" 
                     class="btn btn-primary">
                     <i class="fas fa-comment me-2"></i>ግብረ መልስ ያስገቡ
                  </a>
                `;
